...
export class TelegramClient {
  ...
  private setupMessageHandlers(): void {
    ...
    this.bot.on(callbackQuery('data'), async (ctx) => {
      ...
      if (callbackData === 'read_contract') {
        await ctx.answerCbQuery('You selected "Explain contract".');
        await ctx.telegram.sendMessage(
          ctx.chat.id,
          escapeMarkdownV2('Please send the *smart contract address* (starting with `0x`) and *chain/network* (e.g., Ethereum, Sepolia).'),
          {
            parse_mode: 'MarkdownV2',
          }
        );
      } else if (callbackData === 'get_metadata') {
        await ctx.answerCbQuery('You selected "Get NFT metadata".');
        await ctx.telegram.sendMessage(
          ctx.chat.id,
          escapeMarkdownV2('Please send the *smart contract address* (starting with `0x`), *token ID* and *chain/network* (e.g., Ethereum, Sepolia).'),
          {
            parse_mode: 'MarkdownV2',
          }
        );
